    ...
    def update(
        ...
        for _ in range(cfg.ppo_epochs):
            for mb in iter_minibatches(
                ...
                # GSPO: geometric-mean sequence ratio s_i = exp(mean_t log r_{i,t})
                seq_log_ratio = masked_mean_per_row(log_ratio, mask)
                seq_ratio = torch.exp(seq_log_ratio)

                unclipped = seq_ratio * adv
                clipped_ratio = torch.clamp(seq_ratio, min=clip_low, max=clip_high)
                ...
                clipfrac = (seq_ratio != clipped_ratio).float().mean()
