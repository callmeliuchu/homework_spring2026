...
def compute_per_token_logprobs(
    ...
    *,
    ...
    with torch.set_grad_enabled(enable_grad):
        ...
        log_prbs = torch.gather(logits,1,input_ids[:,1:].reshape(B*L,-1)).reshape(B,L) # B*L, 1
        return log_prbs
...
def approx_kl_from_logprobs(
    ...
    delta = torch.clip(delta,min=-log_ratio_clip,max=log_ratio_clip)
    estimate = torch.exp(delta) - delta - 1
    return  masked_mean(estimate,mask,eps=eps)